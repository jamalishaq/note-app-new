...
const Header = () => {
    const { headerText, setHeaderText } = useHeader();
    const { setDisplaySettings, setFilter, setSelectedTag } = useApp();
    ...
    useEffect(() => {
        if (h1Ref.current) {
            h1Ref.current.innerHTML = headerText || 'All Notes';
        }
    }, [headerText]);


    return (
        <header className="flex flex-col lg:flex-row lg:justify-between col-start-2 lg:items-center lg:px-8 lg:py-4 lg:bg-neutral-0 dark:lg:bg-neutral-950 lg:border-l border-neutral-200 dark:border-neutral-800">
            <div className="lg:hidden bg-neutral-100 dark:bg-neutral-800 w-full px-8 py-4">
                <Logo />
            </div>

            <h1 ref={h1Ref} className="text-neutral-950 dark:text-neutral-0 text-2xl font-bold ml-8 lg:ml-0 mt-4 lg:mt-0 hidden lg:block"></h1>

            <div className="lg:flex items-center gap-4 hidden">
                {/* Seach bar */}
                <SearchBar />
                {/* Settings */}
                <button onClick={handleClick} type="button" className="cursor-pointer text-neutral-950 dark:text-neutral-400">
                    <svg xmlns="http://www.w3.org/2000/svg" width="42" height="42" fill="none" viewBox="0 0 24 24" className="">
