...
const FontTheme = () => {
  const [preferredFont, setPreferredFont] = useState(localStorage.getItem("preferredFont") || "sans");
  const { setActiveTab } = useApp();
  const handleChange = (e: ChangeEvent<HTMLInputElement>) => {
    setPreferredFont(e.target.value);
  };
  ...
  const handleSave = () => {
    if (preferredFont === "sans") {
      ...
      localStorage.setItem("preferredFont", "sans");
      ...
      localStorage.setItem("preferredFont", "monospace");
    }
  };
  ...
  return (
    <div className="px-8 py-6 w-full lg:max-w-lg h-fit">
      ...
      <h1 className="text-neutral-950 dark:text-neutral-0 font-semibold">Font Theme</h1>
      <p className="text-neutral-700 dark:text-neutral-300 text-sm">Choose your font theme:</p>
      <div className="mt-8">
        {/* Sans Serif */}
        <label htmlFor="sans" className={`cursor-pointer flex justify-between items-center p-4 rounded-xl border border-neutral-200 dark:border-neutral-700 ${preferredFont === 'sans' ? 'bg-neutral-100 dark:bg-neutral-800' : ''}`}>
          <span className="flex gap-4">
            {/* Icon Sans Serif */}
            <span className="bg-neutral-0 dark:bg-neutral-950 text-neutral-950 dark:text-neutral-0 size-10 rounded-xl flex justify-center items-center">
            ...
            <span>
              <h2 className="text-neutral-950 dark:text-neutral-0 font-medium text-sm">Sans-serif</h2>
              <p className="text-neutral-700 dark:text-neutral-300 text-xs">Clean and modern, easy to read.</p>
            </span>
          </span>
          {/* Custom radio */}
          <span className={`size-4 rounded-full ${preferredFont === 'sans' ? 'border-4 border-blue-500' : 'border border-neutral-200 '}`}></span>
          <input
            type="radio"
            name="theme"
            id="sans"
            value="sans"
            checked={preferredFont === 'sans'}
            onChange={handleChange} className="sr-only"
          />
        </label>
        {/* Serif */}
        <label htmlFor="serif" className={`cursor-pointer flex justify-between items-center p-4 rounded-xl border border-neutral-200 dark:border-neutral-700 my-4 ${preferredFont === 'serif' ? 'bg-neutral-100 dark:bg-neutral-800' : ''}`}>
          <span className="flex gap-4">
            {/* Icon Serif */}
            <span className="bg-neutral-0 dark:bg-neutral-950 text-neutral-950 dark:text-neutral-0 size-10 rounded-xl flex justify-center items-center">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
            ...
            <span>
              <h2 className="text-neutral-950 dark:text-neutral-0 font-medium text-sm">Serif</h2>
              <p className="text-neutral-700 dark:text-neutral-300 text-xs">Classic and elegant for a timeless feel.</p>
            </span>
          </span>
          {/* Custom radio */}
          <span className={`size-4 rounded-full ${preferredFont === 'serif' ? 'border-4 border-blue-500' : 'border border-neutral-200 '}`}></span>
          <input
            type="radio"
            name="theme"
            id="serif"
            value="serif"
            checked={preferredFont === 'serif'}
            onChange={handleChange} className="sr-only"
          />
        </label>
        {/* Monospace */}
        <label htmlFor="monospace" className={`cursor-pointer flex justify-between items-center p-4 rounded-xl border border-neutral-200 dark:border-neutral-700 ${preferredFont === 'monospace' ? 'bg-neutral-100 dark:bg-neutral-800' : ''}`}>
          <span className="flex gap-4">
            {/* Icon Monospace */}
            <span className="bg-neutral-0 dark:bg-neutral-950 text-neutral-950 dark:text-neutral-0 size-10 rounded-xl flex justify-center items-center">
              <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
            ...
            <span>
              <h2 className="text-neutral-950 dark:text-neutral-0 font-medium text-sm">Monospace</h2>
              <p className="text-neutral-700 dark:text-neutral-300 text-xs">Code-like, great for a technical vibe.</p>
            </span>
          </span>
          {/* Custom radio */}
          <span className={`size-4 rounded-full ${preferredFont === 'monospace' ? 'border-4 border-blue-500' : 'border border-neutral-200 '}`}></span>
          <input
            type="radio"
            name="theme"
            id="monospace"
            value="monospace"
            checked={preferredFont === 'monospace'}
            onChange={handleChange} className="sr-only"
          />
        </label>
      </div>
      <div className="text-right mt-6">
        <button type="button" className="cursor-pointer bg-blue-500 rounded-lg px-4 py-3 text-white text-sm font-medium" onClick={handleSave}>Apply Changes</button>
      </div>
    </div>
  )
}
