...
const Page = () => {
  ...
  return (
    <div className="bg-neutral-0 py-12 md:px-12 px-4 flex flex-col gap-4 rounded-xl lg:w-[540px] md:w-[522px] dark:bg-neutral-950">
      <header className="flex flex-col items-center gap-4">
        <Logo />
        <div className="flex flex-col items-center gap-2">
          <h1 className="text-neutral-950 text-2xl font-bold dark:text-neutral-0">Create Your Account</h1>
          <p className="text-neutral-600 text-sm dark:text-neutral-300">Sign up to start organizing your notes and boost your productivity.</p>
        </div>
      </header>
      <form action={formAction} className="pt-6 flex flex-col gap-4">
        {/* Email Address */}
        <div>
          <label htmlFor="email" id="email" className="block text-sm text-neutral-600 font-medium mb-2 dark:text-neutral-0">Email Address</label>
          <input
            type="text"
            name="email"
            value={email}
            onChange={(e) => setEmail(e.target.value)}
            className={`px-4 py-3 w-full border border-neutral-300 dark:border-neutral-600 text-neutral-950 dark:text-neutral-0 rounded-lg outline-none placeholder:text-neutral-500 placeholder:text-sm ${state?.errors?.email ? "border-red-500 dark:border-red-500" : ""}`} placeholder="email@example.com"
          />
        ...
        <div>
          ...
          <div className={`flex mb-2 border dark:border-neutral-600 border-neutral-300 rounded-lg px-4 py-3  ${state?.errors?.password ? "border-red-500 dark:border-red-500" : ""}`}>
            <input type={showPassword ? "text" : "password"} name="password" value={password} onChange={(e) => setPassword(e.target.value)} className="border-none outline-none grow-1 text-neutral-950 dark:text-neutral-0 text-sm" />
            {!showPassword ? (
          ...
          {state?.errors?.password ? <div className="flex gap-2 items-center text-red-500 text-sm">
            <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M2 8.5C2 11.8133 4.68605 14.5 8 14.5C11.3139 14.5 14 11.8133 14 8.5C14 5.18605 11.3139 2.5 8 2.5C4.68605 2.5 2 5.18605 2 8.5Z" stroke="#FB3748" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M8.0038 10.9621V8.09573V10.9621ZM8 6.0695V6.02734V6.0695Z" fill="#FB3748" />
              <path d="M8.0038 10.9621V8.09573M8 6.0695V6.02734" stroke="#FB3748" strokeLinecap="round" strokeLinejoin="round" />
          ...
          </div> : (<div className="flex gap-2 text-neutral-600 dark:text-neutral-400">
            <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M2 8.5C2 11.8133 4.68605 14.5 8 14.5C11.3139 14.5 14 11.8133 14 8.5C14 5.18605 11.3139 2.5 8 2.5C4.68605 2.5 2 5.18605 2 8.5Z" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M8.0038 10.9621V8.09573V10.9621ZM8 6.0695V6.02734V6.0695Z" fill="currentColor" />
              <path d="M8.0038 10.9621V8.09573M8 6.0695V6.02734" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" />
            </svg>

            <p className=" text-[12px]">At least 8 characters</p>
          </div>)}
        </div>

        <button type="submit" className="text-white font-semibold text-center py-3 bg-blue-500 rounded-lg cursor-pointer hover:bg-blue-700">{pending ? "Saving..." : "Sign up"}</button>
      </form>

      <div>
        <p className="text-center text-neutral-600 text-[14px] dark:text-neutral-300">Or login with:</p>
        <button className="flex justify-center gap-4 py-3 mt-4 cursor-pointer rounded-xl border border-neutral-300 w-full hover:bg-neutral-50 hover:border-neutral-300 dark:bg-none dark:border-neutral-600 text-neutral-950  dark:text-neutral-0 dark:hover:bg-neutral-800 dark:hover:border-neutral-600">
      ...
      <p className="mt-4 text-center text-[14px] text-neutral-600 dark:text-neutral-300">Already have an account? <Link href="/login" className="text-neutral-950 hover:text-blue-500 dark:text-neutral-0">Login</Link></p>
    </div>
  )
}
