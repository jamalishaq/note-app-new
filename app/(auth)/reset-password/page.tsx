"use client"
import { useState } from "react";
// import { signUp } from "../../../lib/auth_actions";
import Logo from "@/app/(note)/components/Logo";
// type SignUpState = {
//   message?: string;
//   errors?: {
//     email?: string[];
//   };
// };



// const initialState: SignUpState = {};

const Page = () => {
  //   const [state, formAction, pending] = useActionState(signUp, initialState);
  const [newPassword, setNewPassword] = useState("");
  const [confirmPassword, setConfirmPassword] = useState("");
  const [isPasswordMatch, setIsPasswordMatch] = useState(true);
  const [showNewPassword, setShowNewPassword] = useState(false);
  const [showConfirmPassword, setShowConfirmPassword] = useState(false);

  const resetPassword = () => {
    if (!(newPassword === confirmPassword)) {
      setIsPasswordMatch(false);
      return;
    }
  };

  return (
    <div className="bg-neutral-0 py-12 md:px-12 px-4  flex flex-col gap-4 rounded-xl lg:w-[540px] md:w-[522px] mx-auto dark:bg-neutral-950">
      <header className="flex flex-col items-center gap-4">
        <Logo />
        <div className="flex flex-col items-center gap-2">
          <h1 className="text-neutral-950 text-2xl font-bold dark:text-neutral-0 ">Reset Your Password</h1>
          <p className="text-neutral-600 text-sm dark:text-neutral-300">Choose a new password to secure your account.</p>
        </div>
      </header>
      <form className="pt-6 flex flex-col gap-4">

        <div>
          <label htmlFor="new_password" id="new_password" className="block mb-2 text-sm text-neutral-600 font-medium dark:text-neutral-0">New Password</label>

          <div className="flex mb-2 border  border-neutral-300  dark:border-neutral-600 rounded-lg px-4 py-3 ">
            <input type={showNewPassword ? "text" : "password"} value={newPassword} name="new_password" onChange={(e) => setNewPassword(e.target.value)} className="border-none outline-none grow-1 text-neutral-950 dark:text-neutral-0 text-sm" />
            {!showNewPassword ? (
              <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="cursor-pointer">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fillRule="evenodd" clipRule="evenodd" d="M10.0022 8.42888C8.89226 8.42888 7.99219 9.2924 7.99219 10.3585C7.99219 11.4239 8.89235 12.2881 10.0022 12.2881C11.112 12.2881 12.0122 11.4239 12.0122 10.3585C12.0122 9.2924 11.1122 8.42888 10.0022 8.42888ZM6.74219 10.3585C6.74219 8.62936 8.20214 7.22888 10.0022 7.22888C11.8023 7.22888 13.2622 8.62936 13.2622 10.3585C13.2622 12.0866 11.8023 13.4881 10.0022 13.4881C8.20201 13.4881 6.74219 12.0866 6.74219 10.3585Z" fill="#717784" />
                  <path fillRule="evenodd" clipRule="evenodd" d="M4.1466 5.99548C5.64193 4.78181 7.70928 3.91687 10.002 3.91687C12.2942 3.91687 14.3615 4.78113 15.857 5.99451C17.3362 7.19468 18.337 8.80808 18.337 10.3585C18.337 11.9089 17.3362 13.5222 15.857 14.7224C14.3615 15.9358 12.2942 16.8001 10.002 16.8001C7.70928 16.8001 5.64193 15.9351 4.1466 14.7214C2.66762 13.521 1.66699 11.9077 1.66699 10.3585C1.66699 8.80928 2.66762 7.19588 4.1466 5.99548ZM4.95364 6.91186C3.64221 7.97626 2.91699 9.28368 2.91699 10.3585C2.91699 11.4333 3.64221 12.7406 4.95364 13.8051C6.24873 14.8562 8.03638 15.6001 10.002 15.6001C11.9673 15.6001 13.755 14.8567 15.0501 13.8059C16.3615 12.7418 17.087 11.4345 17.087 10.3585C17.087 9.28248 16.3615 7.97506 15.0501 6.91102C13.755 5.8602 11.9673 5.11687 10.002 5.11687C8.03638 5.11687 6.24873 5.86073 4.95364 6.91186Z" fill="#717784" />
                </svg>


                <span className="sr-only">Show Password</span>
              </button>
            ) : (
              <button type="button" onClick={() => setShowNewPassword(!showNewPassword)} className="cursor-pointer">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                  <path stroke="#0E121B" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M6.42 17.73c-2.23-1.46-3.67-3.66-3.67-5.59 0-3.28 4.14-7.3 9.25-7.3 2.09 0 4.03.67 5.59 1.71M19.85 8.61c.891 1.13 1.41 2.38 1.41 3.53 0 3.28-4.15 7.3-9.26 7.3-.91 0-1.799-.13-2.63-.36" /><path stroke="#0E121B" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9.766 14.367a3.12 3.12 0 0 1-.925-2.23 3.159 3.159 0 0 1 5.394-2.24M15.11 12.7a3.158 3.158 0 0 1-2.538 2.541M19.892 4.25 4.118 20.024" />
                </svg>
                <span className="sr-only">Hide Password</span>
              </button>
            )}
          </div>

          <div className="flex gap-2 text-neutral-600 dark:text-neutral-400">
            <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
              <path d="M2 8.5C2 11.8133 4.68605 14.5 8 14.5C11.3139 14.5 14 11.8133 14 8.5C14 5.18605 11.3139 2.5 8 2.5C4.68605 2.5 2 5.18605 2 8.5Z" stroke="#525866" strokeLinecap="round" strokeLinejoin="round" />
              <path d="M8.0038 10.9621V8.09573V10.9621ZM8 6.0695V6.02734V6.0695Z" fill="currentColor" />
              <path d="M8.0038 10.9621V8.09573M8 6.0695V6.02734" stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" />
            </svg>

            <p className="text-[12px]">At least 8 characters</p>
          </div>
        </div>
        <div>
          <label htmlFor="confirm_password" id="confirm_password" className="block mb-2 text-sm text-neutral-600 font-medium dark:text-neutral-0">Confirm New Password</label>

          <div className="flex mb-2 border  border-neutral-300 dark:border-neutral-600 rounded-lg px-4 py-3">
            <input type={showConfirmPassword ? "text" : "password"} value={confirmPassword} name="confirm_password" onChange={(e) => setConfirmPassword(e.target.value)} className="border-none outline-none grow-1 text-neutral-950 dark:text-neutral-0 text-sm" />
            {!showConfirmPassword ? (
              <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="cursor-pointer">
                <svg width="20" height="20" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fillRule="evenodd" clipRule="evenodd" d="M10.0022 8.42888C8.89226 8.42888 7.99219 9.2924 7.99219 10.3585C7.99219 11.4239 8.89235 12.2881 10.0022 12.2881C11.112 12.2881 12.0122 11.4239 12.0122 10.3585C12.0122 9.2924 11.1122 8.42888 10.0022 8.42888ZM6.74219 10.3585C6.74219 8.62936 8.20214 7.22888 10.0022 7.22888C11.8023 7.22888 13.2622 8.62936 13.2622 10.3585C13.2622 12.0866 11.8023 13.4881 10.0022 13.4881C8.20201 13.4881 6.74219 12.0866 6.74219 10.3585Z" fill="#717784" />
                  <path fillRule="evenodd" clipRule="evenodd" d="M4.1466 5.99548C5.64193 4.78181 7.70928 3.91687 10.002 3.91687C12.2942 3.91687 14.3615 4.78113 15.857 5.99451C17.3362 7.19468 18.337 8.80808 18.337 10.3585C18.337 11.9089 17.3362 13.5222 15.857 14.7224C14.3615 15.9358 12.2942 16.8001 10.002 16.8001C7.70928 16.8001 5.64193 15.9351 4.1466 14.7214C2.66762 13.521 1.66699 11.9077 1.66699 10.3585C1.66699 8.80928 2.66762 7.19588 4.1466 5.99548ZM4.95364 6.91186C3.64221 7.97626 2.91699 9.28368 2.91699 10.3585C2.91699 11.4333 3.64221 12.7406 4.95364 13.8051C6.24873 14.8562 8.03638 15.6001 10.002 15.6001C11.9673 15.6001 13.755 14.8567 15.0501 13.8059C16.3615 12.7418 17.087 11.4345 17.087 10.3585C17.087 9.28248 16.3615 7.97506 15.0501 6.91102C13.755 5.8602 11.9673 5.11687 10.002 5.11687C8.03638 5.11687 6.24873 5.86073 4.95364 6.91186Z" fill="#717784" />
                </svg>
                <span className="sr-only">Show Password</span>
              </button>
            ) : (
              <button type="button" onClick={() => setShowConfirmPassword(!showConfirmPassword)} className="cursor-pointer">
                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" viewBox="0 0 24 24">
                  <path stroke="#0E121B" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M6.42 17.73c-2.23-1.46-3.67-3.66-3.67-5.59 0-3.28 4.14-7.3 9.25-7.3 2.09 0 4.03.67 5.59 1.71M19.85 8.61c.891 1.13 1.41 2.38 1.41 3.53 0 3.28-4.15 7.3-9.26 7.3-.91 0-1.799-.13-2.63-.36" /><path stroke="#0E121B" strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9.766 14.367a3.12 3.12 0 0 1-.925-2.23 3.159 3.159 0 0 1 5.394-2.24M15.11 12.7a3.158 3.158 0 0 1-2.538 2.541M19.892 4.25 4.118 20.024" />
                </svg>
                <span className="sr-only">Hide Password</span>
              </button>
            )}
          </div>
        </div>

        {!isPasswordMatch && <div className="flex gap-2 items-center text-red-500 text-sm mb-4">
          <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M2 8.5C2 11.8133 4.68605 14.5 8 14.5C11.3139 14.5 14 11.8133 14 8.5C14 5.18605 11.3139 2.5 8 2.5C4.68605 2.5 2 5.18605 2 8.5Z" stroke="#FB3748" strokeLinecap="round" strokeLinejoin="round" />
            <path d="M8.0038 10.9621V8.09573V10.9621ZM8 6.0695V6.02734V6.0695Z" fill="#FB3748" />
            <path d="M8.0038 10.9621V8.09573M8 6.0695V6.02734" stroke="#FB3748" strokeLinecap="round" strokeLinejoin="round" />
          </svg>
          <p>Passsword Not Match</p>
        </div>}

        <button type="button" onClick={resetPassword} className="text-white font-semibold text-center py-3 bg-blue-500 rounded-lg cursor-pointer">{"Reset Password"}</button>
      </form>
    </div>
  )
}

export default Page